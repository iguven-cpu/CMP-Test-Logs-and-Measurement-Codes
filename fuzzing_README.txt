...
fuzzing/
  README.md                     # this file
  text_fields/
    cmp_fuzz.py                 # black-box fuzzer for the 7 CMP text parameter fields
  keyfuzz/
    generate_corpus.ps1         # regenerates all 51 malformed credential files
  results/
    fuzz_findings_summary.json  # anonymized methodology + results
```
...
python text_fields/cmp_fuzz.py
```

Results are written to `results/fuzz_<timestamp>.jsonl` and `.csv`. Run only
...
```powershell
cd keyfuzz
./generate_corpus.ps1            # creates 51 files under ./corpus/
```

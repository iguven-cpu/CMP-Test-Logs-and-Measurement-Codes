...
$dir = Join-Path $PSScriptRoot "fuzzing_corpus"
...
# Derive DER directly from the throwaway PEM (base64-decode its body) so this
# step needs no second openssl call and cannot silently no-op.
$derBytes = [Convert]::FromBase64String((($valid -split "`n" | Where-Object { $_ -notmatch 'BEGIN|END' -and $_.Trim() -ne '' }) -join ''))
Write-Bytes "der_valid.der" $derBytes
$der = $derBytes
...
if ($count -ne 51) { Write-Error "Expected 51 corpus files but produced $count." }
